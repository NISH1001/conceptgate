"""Real-data check: the mixture Concept on GPT-2 activations (weapons concept).
...
Two regimes on data/concepts/weapons.json:
  ...
  A. strict few-shot (last-token rep, ~12 prompts/class) — expects the BIC few-shot
     ...
     BandpassConcept (it IS that gate at J=1), and high LLR rank agreement.
...
def main() -> int:
    ...
    mg = Concept(name=concept["name"]).fit(A_pos, A_neg)
    mg.calibrate_z(3.0)
    ...
    cg = BandpassConcept(name=concept["name"], filter_method="fisher").fit(A_pos, A_neg)
    cg.calibrate_z(3.0)
